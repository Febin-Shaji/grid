...
@pytest.mark.parametrize("n_sided, rolls", [
    ([5, 5000000]),
    ([7, 5000000])
])
def test_double_roll(n_sided, rolls):
    """ 
    Check that the probability for the sum of two n_sided dice matches
    the expected distribution.
    """

    # Store the expected probabilities for the sum of two dice.
    expect = {}
    for x in range(2, 2 * n_sided + 1):
        expect[x] = prob_double_roll(x, n_sided)
...
def prob_double_roll(x, n):
    ...
    """
    # For two n_sided dice, the probability of two rolls summing to x is
